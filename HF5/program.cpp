...
const float BULLET_R = 0.2;
const int BULLET_RESOLUTION = 8; 
...
public:
	Barrel(Transformation transform) : PrimitiveVisibleObject(transform,
		Model(cube_mesh, Material(0.5,0.5,0.5,1, 1,1,1,1, 0,0,0,0, 20), Transformation(Vector3(-0.15, -0.15, 0), Quaternion(), Vector3(0.6, 0.6, 2.5))))
	{
	}
...
private: 
	...
	Vector3 BulletPoint(float alpha, float beta)
	{
		Vector3 P;
		P.X = BULLET_R * sin(alpha)*sin(beta);
		P.Y = BULLET_R * cos(alpha);
		P.Z = BULLET_R * sin(alpha)*cos(beta);
		return P;
	}

public:
	Bullet(float radius0 = BULLET_R )
	{
		radius = radius0;
	...
	void Draw()
	{
		...
		for (int i = 1; i <= BULLET_RESOLUTION; i++)  // 0-180 grad
		{
			PpR = BulletPoint( (i - 1) * PI / BULLET_RESOLUTION , 0.0 );
			PpL = BulletPoint( i * PI / BULLET_RESOLUTION, 0.0 );
			for (int j = 1; j <= 2 * BULLET_RESOLUTION; j++)  // 0-360 grad
			{
				glNormal3f(PpL.X, PpL.Y, PpL.Z);
				glVertex3f(PpL.X, PpL.Y, PpL.Z);
				glNormal3f(PpR.X, PpR.Y, PpR.Z); 
				glVertex3f(PpR.X, PpR.Y, PpR.Z); 
				P = BulletPoint(i * PI / BULLET_RESOLUTION, j * PI / BULLET_RESOLUTION);
				...
				P = BulletPoint((i - 1) * PI / BULLET_RESOLUTION, (j - 1) * PI / BULLET_RESOLUTION);
				glNormal3f(P.X, P.Y, P.Z);
				glVertex3f(P.X, P.Y, P.Z);
				P = BulletPoint((i-1) * PI / BULLET_RESOLUTION, j * PI / BULLET_RESOLUTION); PpR=P;
				glNormal3f(P.X, P.Y, P.Z);
				glVertex3f(P.X, P.Y, P.Z);
				P = BulletPoint(i * PI / BULLET_RESOLUTION, j * PI / BULLET_RESOLUTION);
				glNormal3f(P.X, P.Y, P.Z);
